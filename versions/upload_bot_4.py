...
async def process_upload(channel_link: str, photos_root: Path, pb_compress, lbl_compress,
                         pb_upload, lbl_upload, root, album_delay):
    if not BOT_TOKEN:
    ...
    for i, batch in enumerate(batches, start=1):
        ...
        try:
            ...
            # Обновляем прогресс по каждой фотографии и удаляем файлы
        ...
        # Выполняем задержку только если это не последний альбом
        if i < len(batches):
...
def start_gui():
    def choose_folder():
        ...
        # Получаем значение задержки из поля, проверяем корректность
        try:
        ...
        if folder_path:
            asyncio.run(process_upload(
                ...
                root,
                delay  # Передаем задержку в функцию
            ))
    ...
    tk.Label(root, text="Задержка между альбомами (секунды):").pack(pady=5)
    entry_delay = tk.Entry(root, width=10)
    entry_delay.pack()
    entry_delay.insert(0, str(ALBUM_DELAY_SECONDS))
